# Notion export → `src/` markdown

This document describes how narrative and database content from the Notion export in `external/notion/` is transformed into a **flat** corpus of cross-linked markdown files under `src/`, plus machine-readable metadata for tooling and AI.
...
```bash
python3 -m scripts.notion_to_src
```

To replace all generated `src/*.md` notes in one go:

```bash
python3 -m scripts.notion_to_src --clean
```

- **Inputs**: all `*.md` and `*.csv` under `external/notion/`.
- **Outputs**:
  - `src/*.md` — one file per exported page, plus one index file per database CSV view.
  ...
  - `docs/notion-link-check.txt` — relative `[label](...)` targets in `src/` that do not exist (post-import sanity check).
...
## Design: why everything is in one `src/` folder

- **No per-table or per-plot subdirectories.** Every emitted markdown file lives directly under `src/`. The stable identity of a row is the **32-hex Notion id** in the filename and in YAML front matter, not its former folder in the export. Original Notion **export** files under `external/notion/` keep the exporter’s names; **emitted** names in `src/` are URL-friendly (see below).
...
- Unparseable CSV basenames get a name like `index-{hash}-unparsed.md` derived from a hash of the source path (still under `src/`).

Links in table cells that look like Notion `Label (path.md)` or `Label (path.csv)` are rewritten to point at the **flat** `src/` target (the row’s `.md` or the **generated** index for that `.csv`).
...
- The pipeline:
  ...
  3. If the path does not match (export quirks), falls back to the **32-hex id** in the path to find the target `src/` file.
...
Each **content** note under `src/` (not the `notion-index` files, except as noted for indexes above) is emitted with a leading block, for example:
...
The manifest lists each output filename under `src/` with:
...
- Open **`src/`** as the vault root (or symlink it into a larger vault) for a standard Markdown + relative-link experience.
...
- `scripts/notion_to_src/` — import implementation.
