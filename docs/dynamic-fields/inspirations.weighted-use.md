...
1. Follow outgoing `FEATURES` edges from the inspiration page to feature pages.
2. For each feature page, read the Features database membership edge `(feature)-[:IS_A]->(Features DB)` and its `priority` property.
...
Notion rollup **Weighted Use** (`weighted_use` on `IS_A` edges). No longer depends on Features **Weight** formula snapshots.
