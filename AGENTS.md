...
- The `./content` directory is the **canonical store root**: `content/data/{shard}/{nodeId}.md` per node (YAML frontmatter + markdown body; `{shard}` = first two ULID entropy chars) plus `relationships.json`; `content/model/` holds `associations.json`, `ordered-collections.json`, `schema.json`, `table-schemas.json`, `views.json`, `dynamic-fields.json`, and `extensions.json`.
...
| Task                                 | Do                                                                                                                            | Do not                                           |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| Add or edit nodes, bodies, titles    | Edit `content/data/{shard}/{nodeId}.md` or use the Tome editor / `ContentStore` (`tome-flatfile` / `tome-db` in sibling **tome** repo)                            | Edit `data/tome.sqlite` (or legacy `data/marloth.sqlite`) directly              |
| Add or edit relationships            | Edit `content/data/relationships.json`, `content/model/associations.json`, or use editor / `ContentStore` mutation APIs | Duplicate relationships in node markdown files   |
