...
> **Dev environment:** Open the **silentorb-workbench** devcontainer for editor, tests, and static site builds. VS Code tooling tasks live there; this repo's tasks cover content sync and validation only.
...
- TypeScript domain scripts live under `./scripts/`; Tome packages (`tome-db`, `tome-flatfile`, `tome-sqlite`, `tome-editor`, `tome-static-site`) live in the sibling **`tome`** repository. In silentorb-workbench, open the devcontainer and use the **`tome` Compose service** for `editor:dev` (not this repo directly).
...
| Task                                 | Do                                                                                                                            | Do not                                           |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| Add or edit nodes, bodies, titles    | Edit `content/data/nodes/{shard}/{nodeId}.md` (or `content/archive/nodes/…`) or use the Tome editor / `ContentStore` (`tome-flatfile` / `tome-db` in sibling **tome** repo)                            | Edit `data/tome.sqlite` (or legacy `data/marloth.sqlite`) directly              |
...
| Dynamic property bindings               | Edit `content/model/dynamic-properties.json` or run `bun scripts/seed-dynamic-properties.ts`                                          | Use removed `dynamic_`* SQLite overlay tables    |
...
| Ordered collections, scene order, drag-and-drop reorder | [`../tome/docs/features/ordered-collections.md`](../tome/docs/features/ordered-collections.md) |
| Dynamic properties, computed columns | [`../tome/docs/features/dynamic-properties.md`](../tome/docs/features/dynamic-properties.md) + [`../tome/docs/dynamic-properties/`](../tome/docs/dynamic-properties/) |
