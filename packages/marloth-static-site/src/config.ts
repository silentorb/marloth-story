...
import { defaultDbPathForContent, resolveContentPath } from "marloth-db/content";
...
function buildResolvedConfig(
  ...
  const contentDir = resolvePathArg(
    contentFromCli ?? envString("MARLOTH_CONTENT_PATH", env),
    repoRoot,
  ...
  const dbPath =
    resolvePathArg(dbFromCli, repoRoot) ?? defaultDbPathForContent(contentDir);

  const rawOut = kv.get("out-dir");
  const outFromCli = typeof rawOut === "string" ? rawOut : undefined;
  const outDir =
    resolvePathArg(outFromCli ?? envString("MARLOTH_WEB_OUT_DIR", env), repoRoot) ??
    resolve(repoRoot, "dist/web");

  const rawBase = kv.get("base");
  const baseFromCli = typeof rawBase === "string" ? rawBase : undefined;
  const base = normalizeBase(baseFromCli ?? envString("MARLOTH_WEB_BASE", env) ?? "/");
...
export function printHelp(): void {
  const lines = [
    "marloth-static-site — generate static HTML from content/ nodes",
    "",
    ...
    "  --db-path <path>       SQLite cache path (default: data/marloth.sqlite)",
    "  --out-dir <path>       Output directory (default: dist/web)",
    "  --base <path>          Site base path for embedding (default: /)",
    "  -h, --help",
    "",
    "Environment:",
    "  MARLOTH_CONTENT_PATH   Content directory when --content-dir is omitted",
    "  MARLOTH_DB_PATH        SQLite cache path when --db-path is omitted",
    "  MARLOTH_WEB_OUT_DIR    Output directory when --out-dir is omitted",
    "  MARLOTH_WEB_BASE       Site base path when --base is omitted",
  ];
