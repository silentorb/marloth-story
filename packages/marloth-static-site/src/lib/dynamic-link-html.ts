const NODE_LINK_ICON =
  '<span class="marloth-node-link-icon" aria-hidden="true">' +
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="marloth-node-link-page-icon">' +
...
export function decorateDynamicLinkHtml(html: string, dynamicNodeIds: ReadonlySet<string>): string {
  ...
  return html.replace(/<a\b([^>]*?)>([\s\S]*?)<\/a>/gi, (full, attrs, inner) => {
    if (full.includes("marloth-dynamic-node-link-wrap")) return full;
    ...
    const classAttr = existingClass
      ? ` class="${existingClass} marloth-dynamic-node-link"`
      : ' class="marloth-dynamic-node-link"';
    const attrsWithoutClass = attrs.replace(/\s*class="[^"]*"/i, "");
    return `<span class="marloth-dynamic-node-link-wrap">${NODE_LINK_ICON}<a${attrsWithoutClass}${classAttr}>${inner}</a></span>`;
  });
}
