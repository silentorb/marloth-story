...
export function renderTableCellHtml(
  ...
  if (
    ...
    return `<span class="marloth-database-cell-badge">${escapeHtml(value)}</span>`;
  }
...
export function renderRelationLinksHtml(
  ...
  return links
    .map(
      (link) =>
        `<a href="${nodePagePath(link.targetId, base)}" class="marloth-relation-cell-link">${escapeHtml(link.title)}</a>`,
    )
    .join(" ");
}
