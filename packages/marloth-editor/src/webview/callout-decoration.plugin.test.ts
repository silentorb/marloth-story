...
describe("callout decoration plugin", () => {
  test("applies marloth-callout class to emoji-led blockquotes in the editor DOM", async () => {
    ...
    const callout = root.querySelector("blockquote.marloth-callout");
  ...
  test("does not tag plain blockquotes without a leading emoji", async () => {
    ...
    expect(root.querySelector("blockquote.marloth-callout")).toBeNull();
