...
describe("sidebar-nav", () => {
  test("record links use distinct ids and labels", () => {
    const ids = SIDEBAR_RECORD_LINKS.map((link) => link.id);
    expect(new Set(ids).size).toBe(ids.length);
    expect(SIDEBAR_RECORD_LINKS.map((link) => link.label)).toEqual([
      "Features",
      "Solutions",
      "Scenes",
      "Inspirations",
  ...
  test("Inspirations links to the database, not the parent page", () => {
    const inspirations = SIDEBAR_RECORD_LINKS.find((link) => link.label === "Inspirations");
