...
describe("GraphView", () => {
  ...
  test("shows an error when LOD fetch fails", async () => {
    ...
    await waitFor(() => {
      expect(container.querySelector(".marloth-graph-error")?.textContent).toBe("network down");
    });
  });
  ...
  test("shows a canvas error when 2D rendering is unavailable", async () => {
    ...
    const { GraphView: GraphViewFresh } = await import("./GraphView");
    const api = {
      ...makeMockEditorApi(),
      getGraphExplorerLod: async () => makeGraphLodSnapshot(),
    };
    ...
    await waitFor(() => {
      expect(container.querySelector(".marloth-graph-error")?.textContent).toBe(
