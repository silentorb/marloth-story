...
function NavItem({
  ...
  return (
    <a className={`marloth-side-panel-item${active ? " is-active" : ""}`} href={href} title={title}>
...
export function RecentNodesPanel({
  ...
  return (
    <>
      ...
      <div
        className={`marloth-side-panel-section${collapsed ? " is-collapsed" : ""}`}
        aria-label="Recent nodes"
      >
        <div className="marloth-side-panel-section-label">Recent</div>
        {nodes.map((item) => (
          <NavItem
            key={item.id}
