...
describe("RecentNodesPanel", () => {
  test("renders recent node links with native hrefs", async () => {
    ...
    await waitFor(() => {
      expect(container.querySelectorAll(".marloth-side-panel-item").length).toBe(2);
    });

    const links = Array.from(container.querySelectorAll<HTMLAnchorElement>("a.marloth-side-panel-item"));
    ...
    expect(container.querySelector(".marloth-side-panel-section-label")?.textContent).toBe("Recent");
  });
  ...
  test("renders nothing when the API returns no nodes", async () => {
    ...
    expect(container.querySelector(".marloth-side-panel-section")).toBeNull();
  });
});
