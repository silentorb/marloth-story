...
export function RelationshipTypePicker({
  ...
  return (
    <div
      ref={rootRef}
      className="marloth-record-link-picker is-embedded"
      role="group"
    ...
    >
      <input
        ...
        value={displayValue}
        aria-controls="marloth-relationship-type-picker-listbox"
        onChange={(event) => {
      ...
      />
      <div
        id="marloth-relationship-type-picker-listbox"
        className="marloth-record-link-picker-list"
        role="listbox"
      >
        {error ? <div className="marloth-record-link-picker-error">{error}</div> : null}
        {loading && filtered.length === 0 ? (
          <div className="marloth-record-link-picker-empty">Loading types…</div>
        ) : null}
        {!loading && filtered.length === 0 ? (
          <div className="marloth-record-link-picker-empty">No matching types</div>
        ) : (
          filtered.map((type, index) => {
            ...
            return (
              <button
                ...
                className={`marloth-record-link-picker-item${isActive ? " is-active" : ""}${isSelected ? " is-selected" : ""}`}
                onMouseDown={(event) => event.preventDefault()}
                onClick={() => pick(type)}
              >
                <span className="marloth-record-link-picker-title">
                  {formatRelationshipTypeLabel(type)}
                </span>
                <span className="marloth-record-link-picker-path">{type}</span>
              </button>
            );
          })
        )}
