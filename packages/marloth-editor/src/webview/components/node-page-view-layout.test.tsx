import { readFileSync } from "node:fs";
import { join } from "node:path";
import { describe, expect, test } from "bun:test";

const COMPONENT_DIR = import.meta.dir;

describe("node page markdown editor layout CSS", () => {
  const nodePageCss = readFileSync(join(COMPONENT_DIR, "node-page-view.css"), "utf8");
  const editorCss = readFileSync(join(COMPONENT_DIR, "editor.css"), "utf8");

  test("does not inherit the standalone editor min-height on node pages", () => {
    expect(editorCss).toMatch(/\.marloth-editor-body[\s\S]*min-height:\s*60vh/);
    expect(nodePageCss).toMatch(
      /\.marloth-record-page \.marloth-editor-body[\s\S]*min-height:\s*0/,
    );
    expect(nodePageCss).not.toMatch(
      /\.marloth-record-page \.marloth-editor-body\.is-empty[\s\S]*min-height:\s*0/,
    );
  });

  test("clears Crepe ProseMirror padding on node pages", () => {
    expect(nodePageCss).toMatch(
      /\.marloth-record-page \.marloth-editor-body \.milkdown \.ProseMirror[\s\S]*padding:\s*0/,
    );
  });
});
