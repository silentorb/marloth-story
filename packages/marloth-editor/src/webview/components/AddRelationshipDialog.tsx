...
export function AddRelationshipDialog({
  ...
  return (
    <div
      className="marloth-add-relationship-backdrop"
      onClick={linking ? undefined : onClose}
    >
      <div
        className="marloth-add-relationship-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby={titleId}
        onClick={(event) => event.stopPropagation()}
      >
        <header className="marloth-add-relationship-header">
          <h2 id={titleId} className="marloth-add-relationship-title">
            Relate
          </h2>
          <button
            ref={closeRef}
            type="button"
            className="marloth-add-relationship-close"
            aria-label="Close"
            disabled={linking}
            onClick={onClose}
          >
        ...
        <p className="marloth-add-relationship-hint">
          Link this page to an existing record by relationship type and target.
        </p>
        <div className="marloth-add-relationship-fields">
          <div className="marloth-add-relationship-field">
            <span className="marloth-add-relationship-label">Relationship type</span>
          ...
          <div className="marloth-add-relationship-field">
            <span className="marloth-add-relationship-label">Target record</span>
            {selectedType ? (
              <RecordLinkPicker
                key={targetPickerKey}
                api={api}
              ...
              <div className="marloth-add-relationship-target-placeholder">
        ...
        {linking ? (
          <div className="marloth-add-relationship-status" aria-live="polite">
            Linking…
          </div>
        ) : null}
        {error ? (
          <div className="marloth-add-relationship-error" role="alert">
            {error}
          </div>
        ) : null}
      </div>
