...
export function EnumSelectCell({ def, value, disabled = false, onChange }: EnumSelectCellProps) {
  ...
  return (
    <div className="marloth-enum-select" ref={rootRef}>
      <button
        ref={triggerRef}
        type="button"
        className={[
          "marloth-enum-select-trigger",
          "marloth-database-cell-badge",
      ...
      {open && menuPosition
        ? createPortal(
            <div
              ref={menuRef}
              id={menuId}
              className="marloth-enum-select-menu"
              role="listbox"
              aria-label={def.name}
              style={{
              ...
              {allOptions.map((option, index) => {
                ...
                return (
                  <button
                    ...
                    className={`marloth-enum-select-option${index === activeIndex ? " is-active" : ""}${isSelected ? " is-selected" : ""}`}
                    onMouseEnter={() => setActiveIndex(index)}
                    onClick={() => void selectOption(option)}
                  >
                    <span className="marloth-enum-select-option-label">{option}</span>
                    {isSelected ? (
                      <span className="marloth-enum-select-option-check" aria-hidden="true">
                        ✓
                      </span>
                    ) : null}
                  </button>
