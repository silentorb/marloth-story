...
export function TabEditor({
  ...
  return (
    <form
      className="marloth-table-tab-editor"
      onSubmit={(event) => {
        event.preventDefault();
        onSave({ name: name.trim(), sorts });
      }}
    >
      <div className="marloth-table-tab-name-row">
        <label>
          Tab name
          <input
            value={name}
        ...
        {canDelete ? (
          <button
            type="button"
            className="marloth-table-tab-delete"
            disabled={busy}
            onClick={onDelete}
          >
            Delete
      ...
      <div className="marloth-table-tab-editor-heading">Sort order</div>
      {sorts.map((sort, index) => (
        <div key={`${index}-${sort.column}`} className="marloth-table-tab-sort-row">
          <select
            value={sort.column}
            disabled={busy}
      ...
      <button
        type="button"
        className="marloth-table-tab-sort-add"
        disabled={busy}
      ...
      <div className="marloth-table-tab-editor-actions">
