...
/** Build instance-page Properties from IS_A membership scalars and dynamic fields. */
export function buildPropertiesSection(
  db: GraphDatabase,
  recordId: string,
): PropertiesSection | null {
  // v1: first type membership edge when a page belongs to multiple types.
  let membershipEdge = null as ReturnType<GraphDatabase["listEdgesFromSource"]>[number] | null;
  for (const label of TYPE_MEMBERSHIP_LABELS) {
    const edges = db.listEdgesFromSource(recordId, label);
    if (edges.length > 0) {
      membershipEdge = edges[0]!;
      break;
    }
  }
  if (!membershipEdge) return null;

  const databaseId = membershipEdge.targetId;
  ...
  const storedCells = cellsFromEdgeProperties(membershipEdge.properties);
  ...
  const page = db.getVertex(recordId);
