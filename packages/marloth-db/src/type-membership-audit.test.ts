...
describe("type-membership-audit path matching", () => {
  ...
  test("typeDatabaseTitleFromPath prefers deepest matching database segment", () => {
    db.upsertVertex("insp-db", ["NotionDatabase"], { title: "Inspirations" });
    db.upsertVertex("tr-db", ["NotionDatabase"], { title: "Traversal reasons" });
    db.upsertVertex("feat-db", ["NotionDatabase"], { title: "Features" });
...
describe("type-membership-audit (production graph)", () => {
  ...
  test("typed pages do not store row scalars on the vertex", () => {
    const violations = findVertexScalarsOnTypedPages(db);
